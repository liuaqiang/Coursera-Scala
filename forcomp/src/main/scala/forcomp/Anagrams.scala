...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val occurenceList: List[Occurrences] = occurrences.map(x => (for (i <- 0 to x._2) yield (x._1, i)).toList)
    occurenceList.foldRight(List[Occurrences](Nil))((x, y) => for (i <- x; j <- y) yield i :: j).map(_.filter(_._2!=0))
//    val occurenceList: List[Occurrences] = occurrences.map(x => (for (i <- 1 to x._2) yield (x._1, i)).toList)
//    occurenceList.foldRight(List[Occurrences](Nil))((x, y) => y++(for (i <- x; j <- y) yield i :: j)).filter(_.nonEmpty)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.toMap.foldLeft(x.toMap)( (t,k) => if (t(k._1) == k._2) t - k._1 else t updated (k._1, t(k._1)-k._2)).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsAcc(occs: Occurrences): List[Sentence] =
